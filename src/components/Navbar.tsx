...
export function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[9999] flex justify-center pointer-events-none pt-2 sm:pt-4 px-2 sm:px-4 md:px-8 transition-all duration-500">
      <motion.nav
        ...
        className={cn(
          ...
          scrolled
            ? "w-full sm:max-w-4xl px-3 sm:px-4 py-2 bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-black/15 dark:border-white/15 rounded-xl sm:rounded-full shadow-[0_12px_40px_rgba(0,0,0,0.15)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.5)] hover:shadow-[0_16px_50px_rgba(0,0,0,0.2)] dark:hover:shadow-[0_16px_50px_rgba(0,0,0,0.6)]"
            : "w-full sm:max-w-6xl lg:max-w-7xl px-2 py-2 bg-transparent border border-transparent rounded-none shadow-none",
        )}
      >
        <div className="text-lg sm:text-xl md:text-2xl font-bold tracking-tighter text-black dark:text-white">
          ULEGENDRY.
        </div>

        <div className="hidden md:flex items-center space-x-6 lg:space-x-8 text-sm font-medium text-black/70 dark:text-white/70">
          <a
            href="#services"
